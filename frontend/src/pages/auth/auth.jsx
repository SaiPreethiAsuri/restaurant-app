...
const LoginForm = () => {
  ...
  const handleLogin = (e) => {
    ...
    loginUser("/users/login", { email, password }).then((res) => {console.log('Login Successful with email:', email, 'and password:', password);localStorage.setItem("userDetails",email);navigate('/home')});
...
const RegisterForm = () => {
  ...
  const handleRegister = (e) => {
    ...
    registerUser('/users/signup', reqData).then((res) => { console.log("Registration Successful with email:" + registerData.email);localStorage.setItem(userDetails,reqData);navigate('/home') })
  };
  ...
  return (
    <>
      ...
      <Form className='text-center' onSubmit={handleRegister}>
        <Form.Group className='mb-3' controlId="formBasicEmail">
          <Form.Control type="email" name="email" placeholder="Your Email" value={registerData.email} onChange={handleChange} />
        </Form.Group>
